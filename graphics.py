...
def main():
    ...
    for i, v in enumerate(times):
        axes2[0].text(i, v + 20, f"{v}s", ha='center', va='bottom', fontweight='bold')

    # Panel Memoria
    axes2[1].bar(models, mems, color=colores)
    axes2[1].set_title('Consumo de Memoria')
    axes2[1].set_ylabel('MB')
    axes2[1].axhline(0, color='black', linewidth=0.8)
    for i, v in enumerate(mems):
        offset = 2 if v >= 0 else -6
        axes2[1].text(i, v + offset, f"{v} MB", ha='center', fontweight='bold')

    # Panel CPU
    axes2[2].bar(models, cpus, color=colores)
    axes2[2].set_title('Uso de CPU')
    axes2[2].set_ylabel('%')
    for i, v in enumerate(cpus):
        axes2[2].text(i, v + 0.01, f"{v}%", ha='center', va='bottom', fontweight='bold')

    plt.tight_layout()
    
    # Guardar gráfica de métricas en la carpeta output
    fig2.savefig('output/metricas_rendimiento.png', dpi=300, bbox_inches='tight')
    print("[OK] Gráfica de métricas guardada en 'output/metricas_rendimiento.png'")
    
    # Opcional: mostrar las gráficas también en pantalla
    plt.show()
